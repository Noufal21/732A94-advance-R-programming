...
## [1] 1
## Warning messages:
## 1: In if (number < 0) stop("Error: argument should be positive integer") :
## the condition has length > 1 and only the first element will be used
## 2: In if (number == 0) return(1) :
## the condition has length > 1 and only the first element will be used
## 3: In number:1 : numerical expression has 2 elements: only the first used
...
## KB
## > a$update_food_list(user_id,food_id,update_max)
## Error in a$update_food_list(1,1,5) : 
##  Error: it should be ids
